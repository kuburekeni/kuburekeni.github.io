...
const Cloud = {
  ...
  async pull(code = this.code) {
    ...
    return r && r.data && validSave(r.data) ? r : null;   // ignore saves from the old (v1) version
  },
  ...
  async syncOnBoot() {
    ...
    this.status = 'syncing';
    try {
      if (localStorage.getItem('isekai_cloud_wipe')) { await this.rpc('isekai_delete', { p_code: this.code }); localStorage.removeItem('isekai_cloud_wipe'); }
