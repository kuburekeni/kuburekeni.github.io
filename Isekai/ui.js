...
class StatusScene {
  ...
  draw() {
    ...
    const s = charSprite(m.cls, 'down', 0);
    ctx.fillStyle = '#2a3263'; ctx.fillRect(40, 44, 128, 128);
    ctx.drawImage(s, 40, 44, 128, 128);
    const title = m.cls === 'hero' ? `Otherworlder · ${HERO_CLASSES[G.heroClass || 'blade'].name} · ${INTRO() ? 'Introvert' : 'Extrovert'}` : { lyra: 'Elven Mage · AI companion', garrick: 'Dwarven Guardian · AI companion' }[m.cls];
...
async function partyMenu() {
  ...
  if (!others.length) {
    await say(null, `You travel alone. Your turn timer in battle is ${turnTime()} seconds.`);
    return;
  }
  const i = await list({ x: 196, y: 16, w: 428, title: `Party — turn timer ${turnTime()}s`, items: others.map(m => ({ label: m.name, right: 'Lv ' + m.lvl, desc: `Part ways with ${m.name}? They'll wait for you at ${m.cls === 'lyra' ? 'the Crooked Lantern in Aldmere' : 'the Deep Hearth in Ironhold'}. Fewer companions means more time on your turn in battle.` })) });
  if (i < 0) return;
  const m = others[i];
  if (!(await confirm(null, `Part ways with ${m.name} for now?`))) return;
  await say(m.name, m.cls === 'lyra' ? `…All right. You know where I'll be.` : `Hmph. The Deep Hearth, then. Don't take too long.`, m.cls);
  dismiss(m);
  if (m.cls === 'lyra') G.flags.lyraWaits = true; else G.flags.garrickWaits = true;
  World.syncFollowers();
  toast(`${m.name} left the party. Turn timer: ${turnTime()}s.`, UI.gold);
}
