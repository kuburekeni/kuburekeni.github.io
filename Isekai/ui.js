...
class DialogScene {
  ...
  draw() {
    ...
    if (this.spr) {
      ...
      ctx.drawImage(s, 4, 0, 24, 21, x + 26, y + 22, 80, 70);
    }
...
class ListScene {
  ...
  draw() {
    ...
    if (this.showDesc && cur && cur.desc) {
      const lines = wrap(cur.desc, W - 80, 15);
      const dh = lines.length * 22 + 24;
      drawWindow(16, H - dh - 12, W - 32, dh);
...
class NameEntryScene {
  ...
  draw() {
    const cx = W / 2;
    drawWindow(cx - 200, 150, 400, 150);
    text('What is your name?', cx, 170, UI.gold, 17, 'center');
    ctx.fillStyle = UI.ink; ctx.fillRect(cx - 150, 208, 300, 40);
    ctx.fillStyle = '#2a3263'; ctx.fillRect(cx - 148, 210, 296, 36);
    const caret = Math.floor(TIME * 2) % 2 ? '_' : ' ';
    text(this.name + caret, cx, 218, this.first ? UI.dim : UI.paper, 20, 'center');
    text(Controls.mode === 'keys' ? 'Type a name, then press Enter' : `Press ${Controls.label('ok')} to type your name`, cx, 262, UI.dim, 13, 'center', false);
  }
}
...
function drawMemberCard(m, x, y, w, h, hl) {
  ...
  const s = charSprite(memberKey(m), 'down', 0);
  ...
  if (m.cls !== 'hero') {
    const mo = moraleOf(m.cls);
    text('Morale', x + 96, y + 84, UI.dim, 12);
    bar(x + 152, y + 87, 90, 6, mo, 100, moraleColour(mo));
    text(moraleWord(mo), x + w - 20, y + 82, moraleColour(mo), 13, 'right');
  }
}
...
class StatusScene {
  ...
  draw() {
    ...
    drawWindow(16, 16, W - 32, 448);
    const s = charSprite(memberKey(m), 'down', 0);
    ctx.fillStyle = '#2a3263'; ctx.fillRect(40, 44, 128, 128);
    ctx.drawImage(s, 40, 44, 128, 128);
    const title = m.cls === 'hero'
      ? `${heroTitle()} of House Valen · ${HERO_CLASSES[G.heroClass || 'blade'].name} · ${INTRO() ? 'Introvert' : 'Extrovert'}`
      : `${COMPANIONS[m.cls].title} · fights on their own`;
    ...
    text(`${weaponLabel(m)}: ${ITEMS[m.equip.weapon] ? ITEMS[m.equip.weapon].name : '—'}`, 300, 224, UI.paper, 15, 'left', false);
    ...
    if (m.cls !== 'hero') {
      const mo = moraleOf(m.cls), c = COMPANIONS[m.cls];
      text('Morale', 44, 368, UI.gold, 16);
      bar(44, 394, 206, 10, mo, 100, moraleColour(mo));
      text(moraleWord(mo), 44, 410, moraleColour(mo), 14);
      text('+ ' + c.pro, 300, 368, UI.hp, 13, 'left', false);
      text('−  ' + c.con, 300, 390, UI.bad, 13, 'left', false);
    }
    const next = classOf(m).skills.find(([l]) => l > m.lvl);
    if (next) text(`Next skill at Lv ${next[0]}: ${SKILLS[next[1]].name}`, 44, 424, UI.dim, 14, 'left', false);
    if (G.party.length > 1) text('◀ ▶ switch', W - 40, 424, UI.dim, 13, 'right', false);
  }
}
...
async function itemsMenu() {
  ...
  while (true) {
    ...
    const items = all.map(k => ({ label: ITEMS[k].name, right: '×' + G.inv[k], desc: ITEMS[k].desc + (ITEMS[k].type === 'weapon' || ITEMS[k].type === 'armor' ? '  (Equip it from the Equip menu.)' : ITEMS[k].type === 'key' ? '  (Key item.)' : ''), disabled: ITEMS[k].type !== 'use' || ITEMS[k].battleOnly }));
    const i = await list({ x: 196, y: 16, w: W - 224, items, title: 'Items', index: idx, rows: 10, empty: 'Your bag is empty.' });
    if (i < 0) return;
    idx = i;
...
async function skillsMenu() {
  while (true) {
    ...
    while (true) {
      ...
      const i = await list({ x: 196, y: 16, w: W - 224, items, title: `${m.name} — ${m.mp}/${maxMP(m)} MP` });
      if (i < 0) break;
      const s = SKILLS[sk[i]];
      const amount = t => Math.floor(s.power + stat(m, s.useAtk ? 'atk' : 'mag') * s.scale);
      if (s.target === 'allies') {
...
async function equipMenu() {
  while (true) {
    ...
    while (true) {
      ...
      const items = slots.map(s => ({ label: (s === 'weapon' ? weaponLabel(m) : 'Armor') + ': ' + (ITEMS[m.equip[s]] ? ITEMS[m.equip[s]].name : '—'), desc: ITEMS[m.equip[s]] ? statLine(ITEMS[m.equip[s]]) + ' — ' + ITEMS[m.equip[s]].desc : '' }));
      const si = await list({ x: 196, y: 16, w: W - 224, items, title: `${m.name}   ATK ${stat(m, 'atk')}  DEF ${stat(m, 'def')}  MAG ${stat(m, 'mag')}` });
      if (si < 0) break;
      const slot = slots[si];
      const cands = Object.keys(G.inv).filter(k => ITEMS[k] && ITEMS[k].type === slot && canEquip(m, ITEMS[k]));
      ...
      const ci = await list({ x: 196, y: 16, w: W - 224, items: opts, title: `Change ${slot}`, empty: `No other ${slot}s ${m.name} can use.` });
      if (ci < 0 || !cands.length) continue;
      const newId = cands[ci];
      removeItem(newId);
      if (m.equip[slot]) addItem(m.equip[slot]);
      m.equip[slot] = newId;
      if (m.cls === 'hero') clearHeroSprites();
...
async function questsMenu() {
  ...
  await list({ x: 196, y: 16, w: W - 224, items, title: 'Quests', rows: 7 });
}
...
async function pauseMenu() {
  ...
  let idx = 0;
  while (true) {
    const labels = ['Items', 'Skills', 'Equip', 'Status', 'Party', 'Quests', 'Save', 'Cloud Save', 'Settings', 'Title screen'];
    ...
    else if (L === 'Save') { if (G.map === 'tokyo' || G.map === 'konbini') { toast('Saving begins in the other world.', UI.dim); continue; } saveGame() ? (Sound.sfx('save'), toast('Game saved.', UI.hp)) : toast('Could not save.', UI.bad); }
...
async function shopScene(shopId, keeper, spr) {
  if (shopId !== 'konbini') await say(keeper, pick(['Welcome. Take a look around.', 'What do you need?', 'Stock\'s thin, but it\'s honest.']), spr);
  ...
  while (true) {
    ...
    if (c === 0) {
      ...
      while (true) {
        ...
        const items = stock.map(id => {
          ...
          const ws = it.type === 'weapon' || it.type === 'armor' ? whoCanEquip(it) : null;
          const who = ws ? (ws.length ? '  [Can equip: ' + ws.join(', ') + ']' : '  [Nobody can equip this]') : '';
          return { label: it.name, right: price + ' ' + cur(), disabled: G.gold < price || (it.type === 'key' && itemCount(id) > 0), desc: (it.type !== 'use' ? statLine(it) + ' — ' : '') + it.desc + who + (itemCount(id) ? `  (You have ${itemCount(id)}.)` : '') };
        });
        const i = await list({ x: 196, y: 16, w: W - 224, items, title: 'Buy', index: idx, rows: 10 });
        if (i < 0) break;
        idx = i;
    ...
    if (c === 1) {
      let idx = 0;
      while (true) {
        const ids = Object.keys(G.inv).filter(k => ITEMS[k] && ITEMS[k].type !== 'key');
        const items = ids.map(id => ({ label: ITEMS[id].name, right: `×${G.inv[id]}  ${Math.floor(ITEMS[id].price / 2)} ${cur()}`, desc: ITEMS[id].desc }));
        const i = await list({ x: 196, y: 16, w: W - 224, items, title: 'Sell', index: idx, rows: 10, empty: 'Nothing to sell.' });
...
class GoldPanel {
  ...
  update() { }
  draw() { drawWindow(16, 132, 170, 50); text(`${G.gold} ${cur()}`, 34, 146, UI.gold, 18); }
}
...
function cur() { return G && (G.map === 'tokyo' || G.map === 'konbini') ? '¥' : 'G'; }

// ------------------------------------------------------------------ party (part ways with companions)
async function partyMenu() {
  const others = G.party.slice(1);
  if (!others.length) {
    await say(null, `You travel alone. Your turn timer in battle is ${turnTime()} seconds, and nobody argues with you on the road.`);
    return;
  }
  const i = await list({
    x: 196, y: 16, w: W - 224, rows: 5,
    title: `Party ${G.party.length}/${PARTY_MAX} — turn timer ${turnTime()}s`,
    items: others.map(m => {
      const mo = moraleOf(m.cls), c = COMPANIONS[m.cls];
      return {
        label: m.name, right: `Lv ${m.lvl}  ${moraleWord(mo)}`, color: moraleColour(mo),
        desc: `${c.title}. + ${c.pro}  − ${c.con}  ·  Send ${m.name} home to ${c.homeName}? ` +
          (mo < 45 ? 'They are unhappy with you. Send them away now and they may not be there when you come back.' : 'They will wait for you there. A smaller party means more time on your turn in battle.')
      };
    })
  });
  if (i < 0) return;
  const m = others[i];
  const mo = moraleOf(m.cls);
  if (!(await confirm(null, mo < 45 ? `Send ${m.name} home? They are ${moraleWord(mo).toLowerCase()} — this may be the last you see of them.` : `Send ${m.name} home for now?`))) return;
  await partWith(m);
  toast(`Party ${G.party.length}/${PARTY_MAX}. Turn timer: ${turnTime()}s.`, UI.gold);
}
