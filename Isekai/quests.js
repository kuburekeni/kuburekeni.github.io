...
const CLIENTS = {
  base: ['Farmer Tobbs', 'Hana of Aldmere', 'Innkeeper Suzu', 'Elder Bram', 'Smith Haldor', 'Quartermaster Olga', 'Healer Sera', 'The Aldmere Militia', 'A widow from Thornby', 'Dunstan'],
  door: ['The Eastern Militia', 'Captain Reyes of the Watch', 'A refugee family', 'The Ironhold Council'],
  deal: ['Varek of Emberhollow', 'The Treaty Warden', 'Nyx\'s mother', 'An Ashborn healer', 'The Border Council', 'Ashborn Envoy Karr'],
  purge: ['The Crown Surveyor', 'A returning soldier']
};
const ELITE_NAMES = {
  base: ['Grimtooth', 'the Old Scar', 'Mordrek', 'Blackmaw', 'the Pale One', 'Vex', 'Gorefang', 'Ashhide'],
  door: ['Captain Vessk of the Vanguard', 'Kharn the Burner', 'Warleader Ossa', 'Sergeant Drel'],
  deal: ['Hask, the Treaty-Breaker', 'Lord Aldric\'s Raiders', 'the Border Wolf', 'Captain Merrow (rogue)']
};
const pickClient = () => pick(CLIENTS.base.concat(CLIENTS[G.ending] || [], CLIENTS[G.ending] || []));
const pickElite = () => pick((ELITE_NAMES[G.ending] || []).concat(ELITE_NAMES[G.ending] || [], ELITE_NAMES.base));

function questAreas() {
  return QUEST_AREAS.filter(a =>
    a === 'forest' || a === 'fernhollow'
    || (a === 'mine' && G.flags.mineOpen)
    || ((a === 'wastes' || a === 'castle') && G.flags.throughRift));
}
...
function makeQuest() {
  ...
  const map = MAPS[area];
  const table = encounterTable(area) || map.encounters || [['wolf', 1]];
  let pool = table.filter(([id]) => !ENEMIES[id].boss);
  // after an ending, the jobs follow the state of the war
  if (G.ending === 'door' && Math.random() < 0.55) pool = pool.filter(([id]) => ENEMIES[id].demon).concat([['imp', 1]]);
  if (G.ending === 'deal' && Math.random() < 0.5 && pool.some(([id]) => id === 'bandit')) pool = [['bandit', 1]];
  ...
  if (type === 'gather') {
    ...
    q.itemName = herb;
    q.title = `Gather ${herb}`;
    const why = G.ending === 'deal' && Math.random() < 0.5 ? `for the sick in Emberhollow` : G.ending === 'door' ? `for the wounded coming back from the east` : `for medicine`;
    q.desc = `${client}: "I need ${q.need} ${herb} from ${map.name}, ${why}. They give off a faint glow."`;
  } else if (type === 'hunt') {
    const [enemy] = pick(pool);
    q.enemy = enemy; q.need = irand(3, 6);
    const n = ENEMIES[enemy].name;
    const demon = ENEMIES[enemy].demon, human = ENEMIES[enemy].human;
    q.title = demon ? `Hold back the raiders` : human ? `Keep the peace` : `Thin out the ${n}s`;
    const reason = demon ? pick(['Their raiding party burned a mill on the east road.', 'They took our supply wagons.', 'They attack at night now.'])
      : human ? pick(['Men from the west are attacking Ashborn caravans and trying to break the treaty.', 'Raiders are hitting settlers on both sides of the border.'])
      : pick(['They\'ve been attacking travellers on the road.', 'The herds are getting bolder since the war began.', 'They killed two of my sheep last week.']);
    q.desc = `${client}: "${reason} Defeat ${q.need} ${n}s in ${map.name}."`;
  } else {
    const [enemy] = pick(pool);
    const n = ENEMIES[enemy].name;
    q.enemy = enemy; q.need = 1;
    q.eliteName = ENEMIES[enemy].demon || ENEMIES[enemy].human ? pickElite() : `${pick(['Grimtooth', 'Old Scar', 'Blackmaw', 'Ashhide', 'Gorefang', 'the Pale'])} (${n})`;
    q.mult = G.postgame ? 1.15 : 1;
    q.title = `Bounty: ${q.eliteName}`;
    q.desc = `WANTED: ${q.eliteName}, ${ENEMIES[enemy].demon ? 'an Ashborn raider captain' : ENEMIES[enemy].human ? 'a raider leader breaking the treaty' : 'a huge ' + n}, somewhere in ${map.name}. Look for the red glow. Extremely dangerous.`;
  }
